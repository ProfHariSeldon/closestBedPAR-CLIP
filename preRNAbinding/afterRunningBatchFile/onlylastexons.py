# NAME: onlylastexons.py - select only last exons so those can be cat with the 3' UTR file to make the histogram

# SYNOPSIS: python onlylastexons.py
...
# Named output file: [Named input file 2 trimmed so only binding sites in the last exon of genes are included], because only those are next to the 3' UTR (the exon-3' UTR junction or CDS-3' UTR junction).
...
# Named output file: hg38_QKI_closest_exons_noWeirdChr_PARCLIP_overlap_downneg_onlylastexon.bed
# Output file later used in "cat hg38_QKI_closest_exons_noWeirdChr_PARCLIP_overlap_upneg_lastexon_sorted_dist.bed hg38_QKI_closest_3p_noWeirdChr_PARCLIP_overlap_upneg_sorted_dist.bed > hg38_QKI_closest_3p_noWeirdChr_PARCLIP_overlap_regionstart_lastexon.bed", which is used to create the first half of the 3' UTR histogram.
...
# # Make onlylastexons.py executable in preRNAbinding.s.
...
# # Find the last exons is continued from preRNAbinding.s.
...
# Open the bed file that contains the exons to be removed.
exons_to_remove = open("hg38_exons_rev_lastexons_id.bed","r").readlines()
# Open the bed file that contains the last exon in a gene.
...
with open('hg38_QKI_closest_exons_noWeirdChr_PARCLIP_overlap_downneg_onlylastexon.bed','w') as outfile:
    for lineQKI in ends_of_exons:
        ...
        if found == True:
			# Select only last exons so those can be cat with the 3' UTR file to make the histogram.
